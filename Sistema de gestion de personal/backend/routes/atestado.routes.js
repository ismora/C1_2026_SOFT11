...
router.post("/", async (req, res) => {
  ...
  if (!nombreDocumento || !tipo || !urlArchivo || !idEmpleado) {
    return res.status(400).json({
      mensajeError:
        "Los campos nombre del documento, tipo, URL del archivo y id empleado son obligatorios",
    });
  }
  ...
  if (!tiposPermitidos.includes(tipo)) {
    return res.status(400).json({
      mensajeError: "El tipo debe ser uno de: " + tiposPermitidos.join(", "),
    });
  }
  ...
  try {
    ...
    if (!empleadoExistente) {
      return res
        .status(400)
        .json({ mensajeError: "El empleado especificado no existe" });
    }
  } catch (error) {
    return res
      .status(400)
      .json({ mensajeError: "El ID del empleado no es válido" });
  }

  try {
    const nuevoAtestado = new Atestado({
      nombreDocumento,
      tipo,
      urlArchivo,
